#encoding: utf-8
# this file uesd to explain how deal with the orders issued by different datafeeds
...
class St(bt.Strategy):
    params = dict(
        enter=[1, 3, 4],   # the weekday data enter 
        hold=[7, 10, 15],  # record the exist_bar, holding period over it will be sold
        usebracket=True,
        rawbracket=True,
        pentry=0.015,      # used to calculate the enter limit price
        plimits=0.03,      # used to calculate the enter limit price
        valid=10,          # the valid period of the order(has not been completed)
    )
    ...
    def next(self):
        for i, d in enumerate(self.datas):
            ...
            if not pos and not self.o.get(d, None):  # no market / no orders
                if dt.weekday() == self.p.enter[i]:
                    if not self.p.usebracket:
                        ...
                        p = d.close[0] * (1.0 - self.p.pentry)  # 入场限价
                        pstp = p * (1.0 - self.p.plimits)       # 止损
                        plmt = p * (1.0 + self.p.plimits)       # 止盈
            ...
            # else 不用判断订单的状况，因为就算有pending的订单，不进行处理，之后如果
            # 没完成的话会自动到期的。
