...
# test data with differnt length 
...
class Ctaskn_ind(bt.Indicator):
    ...
    lines = ('skn_ind1','skn_ind2','skn_ind3')

    def __init__(self, window_period, datafeeds):
    ...
    def next(self):
        if self.datas[2].close._idx >= 200:
            print ('ok')
        for i, data in enumerate(self.datas):
            closedata = self.datas[i].get(size = self.params.window_prd,line=0)
            adj_fct = self.datas[i].get(size = self.params.window_prd, line=6)
            adj_closedata = np.array(closedata)*np.array(adj_fct)
            self.lines[i][0] = self.clct_skn_ind(adj_closedata)
            print('skewness indicator of %s is %s'%(self.datas[i]._name,self.lines[i][0]))
...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):        
        ...
        self.skn_ind = Ctaskn_ind(window_period = self.params.skn_window_prd,
                                  datafeeds= self.datas)        
        print('ok')
    def start(self):
...
if __name__ == '__main__':  
    ...
    cerebro.setstartdate('2015-01-01')
    cerebro.setenddate('2017-12-01')
    # Add the Data Feed to Cerebro  
    cerebro.AddMultidata(vtsymbol= ('IC','IF','IH'),filepath= r'D:/backtrader/backtrader-master/CTA_factor_backtrade/mytest_strategy/testdata/' )  
