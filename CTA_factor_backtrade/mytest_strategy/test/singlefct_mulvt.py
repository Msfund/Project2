...
class Ctaskn_ind(bt.Indicator):
    ...
    lines = tuple(map(lambda x : x+'skn_ind', Setting.VTSYMBOL))
    ...
    def next(self):
        for i,data in enumerate(self.datas):
            closedata = data.get(size = self.params.window_prd,line=0)
            adj_fct = data.get(size = self.params.window_prd, line=6)
            adj_closedata = np.array(closedata)*np.array(adj_fct)
            self.lines[i][0] = self.clct_skn_ind(adj_closedata)
            
            print('skewness indicator of %s is %s'%(data._name,self.lines[i][0]))
...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):        
        ...
        self.skn_ind = Ctaskn_ind(window_period = self.params.skn_window_prd, datafeeds= self.datas)
        self.skn_ind.plotinfo.plot = False
    def start(self):
...
if __name__ == '__main__':  
    ...
    cerebro.setstartdate('2015-05-05')
    cerebro.setenddate('2017-12-01')
    # Add the Data Feed to Cerebro  
    cerebro.AddMultidata(vtsymbol= Setting.VTSYMBOL,filepath= r'E:/work/CTA/backtrade/' )  
